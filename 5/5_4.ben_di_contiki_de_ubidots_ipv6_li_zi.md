...
目前，Ubidots 的示例已经合并到 Contiki，且同时在 [George Oikonomou](https://github.com/g-oikonomou/contiki/tree/ubidots-demo) 的仓库中提供了一些函数示例。
...
Ubidots 应用使用 TCP 套接字连接到主机```things.ubidots.com```，主机的 IPv4 和 IPv6 端点配置如下：
<center><img src = "../images/iot_in_five_days/5/image004.png" /> </center>
<center>图 5.4. Ubidots 端点IPv4/IPv6 地址</center>

为了检查程序运行状态，可在```ubidots.c```文件中使能调试打印，搜索```#define DEBUG DEBUG_NONE```，并替换为：

```#define DEBUG DEBUG_PRINT```

默认ubidots 应用程序使用 things.ubidots.com 远程主机，但是如果没有 NAT/NAT64 服务解析主机地址情况下，IPv6终端可明确地定义为：
...
注意，```UBIDOTS_WITH_AUTH_TOKEN```不使用 “” 引号。
现在基本完成了设置，下面编译和烧写 Z1 节点。
```
